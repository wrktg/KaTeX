...
module.exports = function(domTree) {
  ...
  var makeSpan = domTree.makeSpan;
  var makeVList = domTree.makeVList;
  var mathrm = domTree.mathrm;
  var makeSymbol = domTree.makeSymbol;
  var makeFragment = domTree.makeFragment;
  ...
  var groupTypes = {
      ...
      op: function(group, options, prev) {
          ...
          if (group.value.symbol) {
              ...
              for (var i = 1; i < group.value.body.length; i++) {
                  output.push(mathrm(group.value.body[i], group.mode));
              }
      ...
      katex: function(group, options, prev) {
          ...
          var k = makeSpan(
              ["k"], [mathrm("K", group.mode)]);
          var a = makeSpan(
              ["a"], [mathrm("A", group.mode)]);

          a.height = (a.height + 0.2) * 0.75;
          a.depth = (a.height - 0.2) * 0.75;

          var t = makeSpan(
              ["t"], [mathrm("T", group.mode)]);
          var e = makeSpan(
              ["e"], [mathrm("E", group.mode)]);

          e.height = (e.height - 0.2155);
          e.depth = (e.depth + 0.2155);

          var x = makeSpan(
              ["x"], [mathrm("X", group.mode)]);
